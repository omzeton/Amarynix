...
function AmharicPhoneticKeyPress(pressEvent, amharic) {
    ...
    if (window.event) {
        ...
        shiftKey = pressEvent.shiftKey;
        controlKey = pressEvent.ctrlKey;
    }
    ...
    if (keyCode != 16 && keyCode != 8 && keyCode != 13 && keyCode != 0 && keyCode != 32) {
        ...
        if (consonantRetVal != null && consonantRetVal.length > 0) {
            ...
            if (amharicValue == "h" && shiftKey == false) {
                ...
                amharic.value = amharic.value.substring(0, tempStartPos - 1) + String.fromCharCode(hconsValue) + amharic.value.substring(startPos, amharic.value.length);
                ...
                amharic.value = amharic.value.substring(0, startPos) + consonantRetVal + amharic.value.substring(endPos, amharic.value.length);
        ...
        if (offset != 0) {
            ...
            if (amharic.value.length < 1) {
            ...
            } else if (amharic.value.charCodeAt(startPos - 1) == 32 || lastCharPlain == 4959) {
                // if tebeq
                amharic.value = amharic.value + vowelCharacterChar;
                newPos = newPos + 1;
                ...
                amharic.value = amharic.value.substring(0, startPos - 1) + String.fromCharCode(lastCharEtymology) + amharic.value.substring(startPos, amharic.value.length);
            } else {
                var lastchar = amharic.value.charCodeAt(startPos - 1) + offset;

                if (otherVowelCharacter > 0) {
                    amharic.value = amharic.value.substring(0, startPos - 1) + String.fromCharCode(otherVowelCharacter) + amharic.value.substring(startPos, amharic.value.length);
                } else if (isRootLetter(lastCharPlain)) {
                    amharic.value = amharic.value.substring(0, startPos - 1) + String.fromCharCode(lastchar) + amharic.value.substring(startPos, amharic.value.length);
                } else {
                    amharic.value = amharic.value.substring(0, startPos) + vowelCharacterChar + amharic.value.substring(startPos, amharic.value.length);
        ...
        if (amharicValue == "w" && shiftKey == true) {
            //amharicValue == "/") // keycode == 191
            //4775
            ...
            if (!(lastchar2 == 4845 || lastchar2 == 4813 || lastchar2 == 4933 || lastchar2 == 4768 || lastchar2 == 4821)) {
                ...
                if (isRootLetter(lastchar2)) {
                    ...
                    amharic.value = amharic.value.substring(0, startPos - 1) + String.fromCharCode(lastCharModified) + amharic.value.substring(startPos, amharic.value.length);
